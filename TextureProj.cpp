...
const double ZextraDistanceMin = -18.0;
const double ZextraDistanceMax = 18.0;

double XextraDistance = 0.0;              // Extra distance we have moved to/from the scene
double XextraDelta = 0.2;                // Pressing HOME/END moves closer/farther by this amount
const double XextraDistanceMin = -5.0;
const double XextraDistanceMax = 5.0;
...
void mySetViewMatrix() {
    ...
    viewMatrix.Set_glTranslate(XextraDistance, 0.0, -(Zmax + zDistance + ZextraDistance));      // Translate to be in front of the camera
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    case 'M':
        if (mods & GLFW_MOD_SHIFT) {
            ...
            meshRes = meshRes > 5 ? meshRes - 1 : 3;    // Lowercase 'm'
        }
        MyRemeshGeometries();
        return;
    case 'F':
    ...
    case GLFW_KEY_K:
        ...
        break;
    case GLFW_KEY_J:
        XextraDistance += XextraDelta;         // Move farther away from the scene
        ClampMin(&XextraDistance, XextraDistanceMin);
        viewChanged = true;
        break;
    case GLFW_KEY_L:
        XextraDistance -= XextraDelta;         // Move farther away from the scene
        ClampMax(&XextraDistance, XextraDistanceMax);
        viewChanged = true;
        break;
    case GLFW_KEY_A:
...
int main() {
	...
	printf("------------------------------\n");
    printf("Press '1' to '7' to toggle lights.\n");
    printf("Press arrow keys to adjust the view direction.\n");
    printf("Press 'I' 'J' 'K' 'L to translate the view. '\n");
    ...
    printf("Press 'Q' key to toggle viewing the floor and the top only.\n");
